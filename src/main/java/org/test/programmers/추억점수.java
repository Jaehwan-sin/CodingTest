...
public class 추억점수 {
    ...
    public int[] solution(String[] names, int[] scores, String[][] photos) {
        // 이름과 점수를 매핑하기 위한 Map 생성
        Map<String, Integer> scoreMap = new HashMap<>();

        // 이름과 점수를 매핑하여 scoreMap에 저장
        for (int i = 0; i < names.length; i++) {
            scoreMap.put(names[i], scores[i]);
        }
        ...
        for (int i = 0; i < photos.length; i++) {
            ...
            for (String name : photos[i]) {
                // 점수가 존재하면 합산
                sum += scoreMap.getOrDefault(name, 0);
            }
            result[i] = sum;
        }
